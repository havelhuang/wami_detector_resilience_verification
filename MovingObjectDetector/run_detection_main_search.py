...
from MovingObjectDetector.find_neighbours import find_neighbours
from MovingObjectDetector.load_data_run import load_data_run
from MovingObjectDetector.load_data_run_end import load_data_run_end
from MovingObjectDetector.load_data_run_end1 import load_data_run_end1
...
def run_detection_main_search(attack, model_folder, imagefolder, input_image_idx, ROI_centre,
    ...
    # build the tree
    tree = Tree()
    tree.create_node(regressionID, 0, data = [t, ROI_centre, bgt, kf, kf_attack, track_store, track_attack_store, perturbations])  # root node
    node_id_count = 0

    for i in range(t,t+k,1):
        leave_node = tree.leaves(0)
        for leave in leave_node:
            datafile = deepcopy(leave.data)
            neighbors, measurements = find_neighbours(datafile, attack, model_folder, imagefolder, input_image_idx, ROI_window, num_of_template)
            if neighbors.size == 0:
                tree.remove_node(leave.identifier)
            else:
                for ii, regressionID in enumerate(neighbors):
                    node_id_count = node_id_count + 1
                    datafile = deepcopy(leave.data)
                    attack_set = neighbors[:ii]
                    state_data = load_data_run(datafile, regressionID, attack_set, measurements, attack, model_folder, imagefolder, input_image_idx, ROI_window, num_of_template)
                    tree.create_node(regressionID, node_id_count, parent=leave.identifier, data=state_data)

    tree.show()

    # pick up the most deviated track
    res_set = []
    leave_node = tree.leaves(0)
    print("The total paths are ", len(leave_node))
    for leave in leave_node:
        datafile = deepcopy(leave.data)
        # res = load_data_run_end1(datafile, attack, model_folder, imagefolder, input_image_idx, ROI_window, num_of_template, writeimagefolder)
        res = diff_mean(datafile[5], datafile[6])
        res_set.append(res)
    index = np.argmax(res_set)
    leaf = leave_node[index]
    perturbations = leaf.data[7]

    measurement_choice = [tree.get_node(nid).tag for nid in tree.rsearch(leaf.identifier)][::-1]

    print('-------------------------------------------------------------------------------------------------------')
    print(measurement_choice)
    print('-------------------------------------------------------------------------------------------------------')

    # go back and start from the root of tree
    datafile = deepcopy(tree.get_node(0).data)
    load_data_run_end(datafile, measurement_choice, attack, model_folder, imagefolder, input_image_idx, ROI_window, num_of_template, writeimagefolder)
